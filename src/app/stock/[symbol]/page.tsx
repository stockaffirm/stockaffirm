...
export default async function StockPage({
  ...
  const { data: overview } = await supabase
    .from("overview")
    .select("peratio, pegratio, analystratingstrongbuy, analystratingbuy, analystratinghold, analystratingsell, analystratingstrongsell, analysttargetprice")
    .eq("symbol", symbol)
    .single();

  if (!quote || !overview) {
    return <div className="p-4 text-center text-red-600">No data found for {symbol}</div>;
  }
...
<div className="relative inline-block">
  <Link
    ...
    className={`text-4xl font-bold ${textColor} relative group leading-none pl-4`} // <-- add pl-4 here
  >
    {symbol}
  </Link>
  <span
  ...
  >
    {(isUp ? "+" : "") + change.toFixed(2)} ({(isUp ? "+" : "") + percent})
      ...
      <div className="mt-16 text-lg text-gray-700 space-y-2 pl-2">
